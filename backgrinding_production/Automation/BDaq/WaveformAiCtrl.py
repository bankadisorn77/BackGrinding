...
from Automation.BDaq import Scenario, DataMark, ErrorCode, MAX_TRIG_COUNT, BfdAiEventArgs, EventId
...
class WaveformAiCtrl(AiCtrlBase):
    def __init__(self, devInfo = None, hostName = None):
        super(WaveformAiCtrl, self).__init__(Scenario.SceWaveformAi, devInfo, hostName)
    ...
    def addBurnOutHandler(self, proc, userParam):
        self.addEventHandler(EventId.EvtBufferedAiBurnOut, proc, userParam)

    def removeBurnOutHandler(self, proc, userParam):
        self.removeEventHandler(EventId.EvtBufferedAiBurnOut, proc, userParam)
    ...
    def release(self):
        TWaveformAiCtrl.Release(self._obj)
